...
# ==========================================
# PARAMETRI DI DEFAULT (usati se eseguito standalone)
# ==========================================
...
# ==========================================
# FUNZIONE PRINCIPALE — chiamabile dal MAB
# ==========================================

def run(params, genera_grafici=True, epoch_label=None, cartella_padre=None):
    """
    Lancia una singola simulazione ns-3 con i parametri forniti.

    Parametri
    ---------
    params : dict
        Dizionario con tutti i parametri della simulazione.
    genera_grafici : bool
        Se True genera overview + per_sta.
        Impostare False durante il loop MAB per velocizzare le epoch.
    epoch_label : str | None
        Stringa aggiunta al nome della sottocartella (es. "epoch_03_arm2").
    cartella_padre : str | None
        Se fornita, la cartella della run viene creata DENTRO cartella_padre.
        Usata dal MAB per raccogliere tutto in MAB_<timestamp>/.
        Se None, viene creata in Risultati/ come in modalità standalone.

    Ritorna
    -------
    metriche : dict  oppure  None in caso di errore
        Contiene:
          — metriche aggregate (medie su tutte le STA)
          — metriche per-STA (liste, una voce per STA)
          — params usati in questa run
          — timestamp e cartella
        Tutto quello che serve per validare i risultati.
    """

    # ── Generazione ID e cartella ──────────────────────────────────────────
    ...
    nome_run = f"Run_{timestamp}_STA-{params['nStations']}_TWT-{twt_label}ms"
    if epoch_label:
        nome_run += f"_{epoch_label}"

    if cartella_padre:
        folder_name = os.path.join(cartella_padre, nome_run)
    else:
        folder_name = os.path.join("Risultati", nome_run)
    ...
    if not os.path.exists(csv_file_generato):
    ...
    def col_mean(name):
        return float(df_mean.get(name, 0))
    ...
    pktloss    = col_mean('pktloss_perc')
    energy_j   = col_mean('energymodel_total_energy_J')

    # ── Metriche complete da restituire ────────────────────────────────────
    # Contiene TUTTO: aggregate, per-STA, params usati, metadati.
    # Questo dict viene salvato come JSON nella cartella della run
    # ed è quello che si manda per validare i risultati.
    metriche = {
        # — metadati run —
        "timestamp":    timestamp,
        "sim_id":       sim_id_univoco,
        "cartella":     folder_name,
        "epoch_label":  epoch_label,

        # — params usati in questa run —
        "params": {k: (list(v) if isinstance(v, list) else v) for k, v in params.items()},

        # — metriche aggregate (medie su tutte le STA) —
        "aggregate": {
            "sleep_time_perc":       sleep_t,
            "idle_time_perc":        idle_t,
            "rx_time_perc":          rx_t,
            "tx_time_perc":          tx_t,
            "cca_time_perc":         cca_t,
            "sleep_energy_perc":     sleep_e,
            "idle_energy_perc":      idle_e,
            "rx_energy_perc":        rx_e,
            "tx_energy_perc":        tx_e,
            "cca_energy_perc":       cca_e,
            "avg_lat_ms":            latenza,
            "avg_effective_lat_ms":  lat_eff,
            "throughput_Mbps":       throughput,
            "pktloss_perc":          pktloss,
            "total_energy_J":        energy_j,
        },

        # — metriche per-STA (liste ordinate per STA 1..N) —
        "per_sta": {
            "sta_ids":             sta_labels,
            "sleep_time_perc":     sleep_t_sta,
            "idle_time_perc":      idle_t_sta,
            "rx_time_perc":        rx_t_sta,
            "tx_time_perc":        tx_t_sta,
            "cca_time_perc":       cca_t_sta,
            "sleep_energy_perc":   sleep_e_sta,
            "idle_energy_perc":    idle_e_sta,
            "rx_energy_perc":      rx_e_sta,
            "tx_energy_perc":      tx_e_sta,
            "cca_energy_perc":     cca_e_sta,
            "avg_lat_ms":          lat_sta,
            "avg_effective_lat_ms": lat_eff_sta,
            "throughput_Mbps":     tput_sta,
            "total_energy_J":      energy_j_sta,
            "pktloss_perc":        pktloss_sta,
        },

        # — alias diretti usati dal MAB per calcolare la reward —
        "idle_energy_perc":      idle_e,
        "sleep_time_perc":       sleep_t,
        "avg_lat_ms":            latenza,
        "avg_effective_lat_ms":  lat_eff,
    ...
    # ── Salvataggio metriche complete come JSON ────────────────────────────
    json_path = f"{folder_name}/metriche.json"
    with open(json_path, "w") as f:
        json.dump(metriche, f, indent=2)
    print(f"📄 Metriche complete salvate: {json_path}")

    # ── Stampa terminale ───────────────────────────────────────────────────
    print(f"✅ COMPLETATO! Cartella: {folder_name}")
    print(f"📊 Media -> SLEEP: {sleep_t:.1f}% tempo | IDLE energia: {idle_e:.1f}% | Latenza: {latenza:.2f} ms | PktLoss: {pktloss:.2f}%")

    if n_sta > 1:
        print("\n📋 Riepilogo per STA:")
        print(f"{'STA':<8} {'Sleep%t':>8} {'Idle%e':>8} {'Energia(mJ)':>12} {'Lat(ms)':>9} {'PktLoss%':>10}")
        print("-" * 60)
        for i, sta in enumerate(sta_labels):
            print(f"{sta:<8} {sleep_t_sta[i]:>7.1f}% {idle_e_sta[i]:>7.1f}% "
                  f"{energy_j_sta[i]*1000:>11.1f} {lat_sta[i]:>8.1f} {pktloss_sta[i]:>9.2f}%")
    ...
    x_sta = range(n_sta)
    bar_w = max(0.12, min(0.35, 0.8 / n_sta))
    ...
    if n_sta > 1:
        ...
        ax.legend(fontsize=9)
        ax.set_ylim(0, 110)
        ax.axhline(y=sum(sleep_t_sta) / n_sta, color='#5DCAA5', linestyle='--', alpha=0.5, linewidth=1)
        ax.axhline(y=sum(idle_e_sta) / n_sta,  color='#378ADD', linestyle='--', alpha=0.5, linewidth=1)
        ax.grid(axis='y', alpha=0.3)
        ...
        bars = ax.bar(list(x_sta), energy_j_sta, bar_w * 1.8, color=colors_energy, edgecolor='white')
        ...
        bars = ax.bar(list(x_sta), pktloss_sta, bar_w * 1.8, color=colors_loss, edgecolor='white')
        ...
        ax = axes2[2, 1]
        ax.scatter(pktloss_sta, energy_j_sta, c=['#378ADD'] * n_sta, s=120, zorder=5)
        for i, label in enumerate(sta_labels):
            ax.annotate(label, (pktloss_sta[i], energy_j_sta[i]),
...
# ==========================================
# ESECUZIONE STANDALONE (python run_esperimento.py)
# ==========================================

if __name__ == "__main__":
    metriche = run(DEFAULT_PARAMS, genera_grafici=True)
    if metriche:
        print(f"\n📦 Metriche aggregate: {metriche['aggregate']}")